...
- Entrenar **BERTopic** sobre el **corpus de entrenamiento completo** (1,6M tweets), como exige la consigna para cualquier modelo entrenado. `min_cluster_size` de HDBSCAN se sube de 10 a 200 (mismo factor de escala que el corpus) para no terminar con decenas de miles de micro-tópicos, y los embeddings se cachean en disco por el costo de cómputo.
- Exportar la tabla de tópicos resultante (id, tamaño, nombre, palabras clave) como objeto propio en `data/processed/topicos_train.csv` (sección 2.1 del notebook).
...
- Visualizar la **jerarquía de tópicos** (dendrograma, `visualize_hierarchy()`) para ver con qué otros tópicos se agrupa Irán: en el corpus completo se agrupa con un clúster de contenido tipo media/reportería (libros, blogs, fotos, videos) y estilo personal (ropa, pelo) — no con otros tópicos de "crisis" (no hay entre los más grandes). Es distinto de lo que daba la corrida sobre 80k tweets (quejas cotidianas: dolor de cabeza, aburrimiento, lluvia, dentista): la jerarquía agrupa por similitud de vocabulario (`c-TF-IDF`), no por tema estricto, y qué comparte vocabulario con Irán depende de la composición completa de tópicos de cada corrida.
- BERTopic usa cosine similarity internamente (c-TF-IDF y UMAP), por lo que también aporta a la métrica obligatoria.

**Aclaración importante:** los tópicos dominantes por volumen del corpus **no son políticos** — son charla cotidiana de Twitter (clima/lluvia, música, Twitter, dinero, pelo/belleza). Irán no es "el tópico principal" del dataset; es el caso de éxito elegido entre los eventos candidatos para poner a prueba la metodología de detección de eventos por picos de volumen.

**Resultado (corpus completo, 1.580.124 tweets, 722 tópicos):** de los dos eventos puntuales buscados específicamente por caer dentro del rango de fechas del dataset, solo uno dejó una huella detectable como tópico propio. El tópico de la elección/protestas en Irán (id 127, 1.521 tweets) es prácticamente inexistente antes del 12/6/2009 (95 tweets) y explota después (1.426 tweets), con 83,0% de tweets negativos frente al ~50% esperado por el balance del dataset — un pico real y verificable. El tamaño del tópico escaló ~20x respecto a la corrida sobre 80k tweets (77 → 1.521 tweets), el mismo factor de escala del corpus, validando el ajuste de `min_cluster_size` (10 → 200). La hipótesis inicial sobre la muerte de Michael Jackson (25/6/2009, límite superior del dataset) **no se confirmó**: no llegó a formar un tópico propio (154 menciones en el corpus completo, todas previas al hecho) porque la recolección del dataset corta a las 10:28 (PDT) de ese día, ~4 horas antes de que se anunciara su muerte (~14:26 PDT) — se documenta como hallazgo honesto en vez de forzar una conclusión que los datos no respaldan.
